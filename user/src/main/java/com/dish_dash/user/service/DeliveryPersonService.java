...
public class DeliveryPersonService {
    ...
    public boolean setLocation(LocationDto locationDto, long deliveryPersonId) {
        return locationRepository.findById(locationDto.getId()).map(location -> {
    ...
    public LocationDto getLocation(long deliveryPersonId) {
        return locationRepository.findById(deliveryPersonId).map(UserMapper.INSTANCE::locationToDto).orElse(null);
    }
}
